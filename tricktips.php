...
elseif(!is_numeric($manobra)) { header("Location: tricktips.php");}
include("topo.php");
...
$result = mysql_query("SELECT * FROM tricktips  WHERE id='".$manobra."'");
$contarids = mysql_query ( "SELECT * FROM tricktips WHERE id='".$manobra."'" );
...
<li><a href="fotofilme.php">Foto/Filme</a></li>
	...
	<div id="page">
      ...
      <div id="page-bgbtm">
        <h2> tricktips. </h2>
<p> Encontra nesta secção os videos com as dicas necessárias para que consigas aterrrar uma manobra dificil. Os melhores conselhos de skaters que dominam a manobra em questão. A manobra que estás a tentar aprender não está aqui? Envia sugestoes para o nosso endereço de email <a href="#">geral@unitedskating.com </a> e o unitedskating vai tratar de criar um tricktip para a manobra que desejas. Esta sec&ccedil;&atilde;o &eacute; actualizada regularmente.<br /> Podes <b>deixar o teu comentário</b> acerca da manobra <b>mais abaixo!</b></p>
  ...
  <div id="bg1a">
		<div id="bg2a">
			<div id="bg3a">
                ...
                <?php
if($manobranull=="true") {
$result = mysql_query("SELECT * FROM tricktips  ORDER BY id DESC LIMIT 1");
$row = mysql_fetch_array($result);
echo '<h2> '.$row["nome"].' - '.$row["manobra"].' </h2> <p>'.$row["embed"].'</p>';
						}
elseif($idsnull=="true") {
echo "<h2>ID especificado não existe.</h2>"; 
} else {
$row = mysql_fetch_array($result);
echo '<h2> '.$row["nome"].' - '.$row["manobra"].' </h2> <p>'.$row["embed"].'</p>';
}
...
if($manobra!="") {$id = $_GET["skaterid"]; } else {
$resultget = mysql_query("SELECT * FROM tricktips  ORDER BY id DESC LIMIT 1");
$rowget = mysql_fetch_array($resultget);
$id = $rowget["id"];
}
$result2 = mysql_query("SELECT * FROM `comments` WHERE post='$id' and modo='tricktips' ORDER BY id DESC"); 
if(!$result2) { echo "erro de selecionar tabela";}
$numcom2 = mysql_query ( "SELECT count(*) from comments WHERE post='$id' and modo='tricktips'" );
 list ( $numcom ) = mysql_fetch_row ( $numcom2 );
$postid = $row['id'];
?>

				<a name="co"></a>
				<h3 class="title">Coment&aacute;rios na manobra "<?php echo $row["manobra"]; ?>" (<?php echo "$numcom"; ?>) </h2>
				<a name="co2"></a>
				  <?php
				$idco = $_GET["idco"];
...
<!--
window.location = "tricktips.php?skaterid='.$id.'&b=apagado#co"
//-->
</script>';
						}
			  if($b=="apagar" and isset($_COOKIE["admin"]) and $passadmin==$_COOKIE["admin"]) {
					    ...
					    mysql_query("DELETE FROM comments WHERE id='".$idco."'" );
					 $numcom2a = mysql_query ( "SELECT count(*) from comments WHERE post='". $id . "' and modo='tricktips'" ); 
...
<!--
window.location = "tricktips.php?skaterid='.$id.'&b=apagado2#co"
//-->
</script>';
...
?>
       <script type="text/javascript">
		...
		window.location = "tricktips.php?skaterid=<?php echo $id; ?>&b=apagar&idco=<?php echo $row2["id"]; ?>";
...
?>
       <script type="text/javascript">
		...
		window.location = "tricktips.php?skaterid=<?php echo $id; ?>&b=apagar2&idco=<?php echo $row2["id"]; ?>";
 ...
 echo "<hr />";
  ...
  if($numcom == "0") { echo "Sem comentários, seja o primeiro!<br>";}
				...
				$numcom6 = mysql_query ( "SELECT count(*) from comments WHERE ip='".$ip."' and modo='tricktips'" );
 list ( $numcom8 ) = mysql_fetch_row ( $numcom6 );
 $result6 = mysql_query("SELECT * FROM comments WHERE ip='$ip' and modo='tricktips' ORDER BY id DESC");
...
VALUES ('$id', '$nomecomment', '$emailcomment', '$textcomment', '$webcomment', '$data', '$hora', '$ip', '$timeAfterOneHour', 'tricktips')");
...
<!--
window.location = "tricktips.php?skaterid='.$id.'&env=sim#co2"
//-->
</script>';
...
VALUES ('$id', '$nomee', '$userid', '$textcomment',  '$data', '$hora', '$ip', '$timeAfterOneHour', 'user', 'tricktips' )");
...
$linharr = mysql_fetch_array($arrayr);
                     ...
                     <form id="form1" method="post" action="tricktips.php?skaterid=<?php echo $id; ?>#co3">&nbsp;
...
VALUES ('$id', 'admin', '$textcomment',  '$data', '$hora', '$ip', '$timeAfterOneHour', 'admin', 'tricktips' )");
...
$linhar = mysql_fetch_array($array);
         ...
         <form id="form1" method="post" action="tricktips.php?skaterid=<?php echo $id; ?>#co2">
				  ...
				  <form id="form1" method="post" action="tricktips.php?skaterid=<?php echo $id; ?>#co4">
