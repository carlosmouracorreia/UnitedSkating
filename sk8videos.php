...
elseif(!is_numeric($manobra)) { header("Location: sk8videos.php");}
include("topo.php");
...
$result = mysql_query("SELECT * FROM sk8videos  WHERE id='".$manobra."'");
$contarids = mysql_query ( "SELECT * FROM sk8videos WHERE id='".$manobra."'" );
...
<li><a href="fotofilme.php">Foto/Filme</a></li>
	...
	<div id="page">
	  <div id="page-bgtop">
        <h2> sk8videos. </h2>
<p> Nesta secção podes encontrar os melhores vídeos de skate, disponíveis para ver no nosso website, online. Nacionais ou internacionais, este é o sitio com horas de entertenimento garantido! O vídeo que queres ver não está aqui? Envia-nos um email para <a href="#">geral@unitedskating.com</a> com as tuas sugestões. <br  /> Link quebrado? envia nos um email e nós repomos o vídeo em questão. <br  />  <b>Atenção:</b> O unitedskating.com não hospeda qualquer contéudo ilegal ou protegido por direitos de autor, todos os vídeos estão hospedados noutros websites, não somos responsáveis por qualquer violação de copyright.<br /> Podes <b>deixar o teu comentário</b> sobre o filme <b>mais abaixo!</b></p>
		...
		<div id="bg2a">
			<div id="bg3a">
				...
				<div id="contento3">
					<?php
if($manobranull=="true") {
$result = mysql_query("SELECT * FROM sk8videos ORDER BY id DESC LIMIT 1");
$row = mysql_fetch_array($result);
echo '<h3> '.$row["filme"].' </h3> <p>'.$row["embed"].'</p>';
						}
elseif($idsnull=="true") {
echo "<h3>ID especificado não existe.</h3>"; 
} else {
$row = mysql_fetch_array($result);
echo '<h3> '.$row["filme"].' </h3> <p>'.$row["embed"].'</p>';
}
...
$resultget = mysql_query("SELECT * FROM sk8videos  ORDER BY id DESC LIMIT 1");
$rowget = mysql_fetch_array($resultget);
$id = $rowget["id"];
}
$result2 = mysql_query("SELECT * FROM `comments` WHERE post='$id' and modo='sk8videos' ORDER BY id DESC"); 
if(!$result2) { echo "erro de selecionar tabela";}
$numcom2 = mysql_query ( "SELECT count(*) from comments WHERE post='$id' and modo='sk8videos'" );
 list ( $numcom ) = mysql_fetch_row ( $numcom2 );
$postid = $row['id'];
?>

				<a name="co"></a>
				<h3 class="title">Coment&aacute;rios em <?php echo $row["filme"]; ?> (<?php echo "$numcom"; ?>) </h2>
				<a name="co2"></a>
				  <?php
				$idco = $_GET["idco"];
...
<!--
window.location = "sk8videos.php?videoid='.$id.'&b=apagado#co"
//-->
</script>';
						}
			  if($b=="apagar" and isset($_COOKIE["admin"]) and $passadmin==$_COOKIE["admin"]) {
					    ...
					    mysql_query("DELETE FROM comments WHERE id='".$idco."'" );
					 $numcom2a = mysql_query ( "SELECT count(*) from comments WHERE post='". $id . "' and modo='sk8videos'" ); 
...
<!--
window.location = "sk8videos.php?videoid='.$id.'&b=apagado2#co"
//-->
</script>';
...
?>
       <script type="text/javascript">
		...
		window.location = "sk8videos.php?videoid=<?php echo $id; ?>&b=apagar&idco=<?php echo $row2["id"]; ?>";
...
?>
       <script type="text/javascript">
		...
		window.location = "sk8videos.php?videoid=<?php echo $id; ?>&b=apagar2&idco=<?php echo $row2["id"]; ?>";
 ...
 echo "<hr />";
  ...
  if($numcom == "0") { echo "Sem comentários, seja o primeiro!<br>";}
				...
				$numcom6 = mysql_query ( "SELECT count(*) from comments WHERE ip='".$ip."' and modo='sk8videos'" );
 list ( $numcom8 ) = mysql_fetch_row ( $numcom6 );
 $result6 = mysql_query("SELECT * FROM comments WHERE ip='$ip' and modo='sk8videos' ORDER BY id DESC");
...
VALUES ('$id', '$nomecomment', '$emailcomment', '$textcomment', '$webcomment', '$data', '$hora', '$ip', '$timeAfterOneHour', 'sk8videos')");
...
<!--
window.location = "sk8videos.php?videoid='.$id.'&env=sim#co2"
//-->
</script>';
...
VALUES ('$id', '$nomee', '$userid', '$textcomment',  '$data', '$hora', '$ip', '$timeAfterOneHour', 'user', 'sk8videos' )");
...
$linharr = mysql_fetch_array($arrayr);
                     ...
                     <form id="form1" method="post" action="sk8videos.php?videoid=<?php echo $id; ?>#co3">&nbsp;
...
VALUES ('$id', 'admin', '$textcomment',  '$data', '$hora', '$ip', '$timeAfterOneHour', 'admin', 'sk8videos' )");
...
$linhar = mysql_fetch_array($array);
         ...
         <form id="form1" method="post" action="sk8videos.php?videoid=<?php echo $id; ?>#co2">
				  ...
				  <form id="form1" method="post" action="sk8videos.php?videoid=<?php echo $id; ?>#co4">
