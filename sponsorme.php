...
elseif(!is_numeric($manobra)) { header("Location: sponsorme.php");}
include("topo.php");
...
$result = mysql_query("SELECT * FROM sponsorme  WHERE id='".$manobra."'");
$contarids = mysql_query ( "SELECT * FROM sponsorme  WHERE id='".$manobra."'" );
...
<li class="current_page_item2"><a href="fotofilme.php">Foto/Filme</a></li>
	...
	<div id="page">
	  <div id="page-bgtop">
        <h2> sponsor me. </h2>
<p> Nesta secção estão presentes clips de skaters que pretendem ser patrocionados/apoiados e estão a um nível de experiencia adequado. Alguns dos vídeos foram editados pelo unitedskating.com , embora a maioria seja editada pelos próprios skaters. Se é uma skateshop/marca/entidade e pertende obter mais informações sobre um atleta que lhe interesse envie-nos um email para o <a href="#">geral@unitedskating.com</a>. <br /> Podes <b>deixar o teu comentário</b> do skater <b>mais abaixo!</b></p>
...
<?php
if($manobranull=="true") {
$result = mysql_query("SELECT * FROM sponsorme  ORDER BY id DESC LIMIT 1");
$row = mysql_fetch_array($result);
echo '<h3> '.$row["nome"].' - '.$row["localidade"].' </h3> <p>'.$row["embed"].'</p>';
						}
elseif($idsnull=="true") {
...
$resultget = mysql_query("SELECT * FROM sponsorme  ORDER BY id DESC LIMIT 1");
$rowget = mysql_fetch_array($resultget);
$id = $rowget["id"];
}
$result2 = mysql_query("SELECT * FROM `comments` WHERE post='$id' and modo='sponsorme' ORDER BY id DESC"); 
if(!$result2) { echo "erro de selecionar tabela";}
$numcom2 = mysql_query ( "SELECT count(*) from comments WHERE post='$id' and modo='sponsorme'" );
...
<!--
window.location = "sponsorme.php?skaterid='.$id.'&b=apagado#co"
//-->
</script>';
						}
			  if($b=="apagar" and isset($_COOKIE["admin"]) and $passadmin==$_COOKIE["admin"]) {
					    ...
					    mysql_query("DELETE FROM comments WHERE id='".$idco."'" );
					 $numcom2a = mysql_query ( "SELECT count(*) from comments WHERE post='". $id . "' and modo='sponsorme'" ); 
...
<!--
window.location = "sponsorme.php?skaterid='.$id.'&b=apagado2#co"
//-->
</script>';
...
?>
       <script type="text/javascript">
		...
		window.location = "sponsorme.php?skaterid=<?php echo $id; ?>&b=apagar&idco=<?php echo $row2["id"]; ?>";
...
?>
       <script type="text/javascript">
		...
		window.location = "sponsorme.php?skaterid=<?php echo $id; ?>&b=apagar2&idco=<?php echo $row2["id"]; ?>";
 ...
 echo "<hr />";
  ...
  if($numcom == "0") { echo "Sem comentários, seja o primeiro!<br>";}
				...
				$numcom6 = mysql_query ( "SELECT count(*) from comments WHERE ip='".$ip."' and modo='sponsorme'" );
 list ( $numcom8 ) = mysql_fetch_row ( $numcom6 );
 $result6 = mysql_query("SELECT * FROM comments WHERE ip='$ip' and modo='sponsorme' ORDER BY id DESC");
...
VALUES ('$id', '$nomecomment', '$emailcomment', '$textcomment', '$webcomment', '$data', '$hora', '$ip', '$timeAfterOneHour', 'sponsorme')");
...
<!--
window.location = "sponsorme.php?skaterid='.$id.'&env=sim#co2"
//-->
</script>';
...
VALUES ('$id', '$nomee', '$userid', '$textcomment',  '$data', '$hora', '$ip', '$timeAfterOneHour', 'user', 'sponsorme' )");
...
$linharr = mysql_fetch_array($arrayr);
                     ...
                     <form id="form1" method="post" action="sponsorme.php?skaterid=<?php echo $id; ?>#co3">&nbsp;
...
VALUES ('$id', 'admin', '$textcomment',  '$data', '$hora', '$ip', '$timeAfterOneHour', 'admin', 'sponsorme' )");
...
$linhar = mysql_fetch_array($array);
         ...
         <form id="form1" method="post" action="sponsorme.php?skaterid=<?php echo $id; ?>#co2">
				  ...
				  <form id="form1" method="post" action="sponsorme.php?skaterid=<?php echo $id; ?>#co4">
